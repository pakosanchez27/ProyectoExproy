...
$sqlCertificado = "SELECT * FROM certificaciones WHERE id_certificacion = $idCertificacion ";
$resultCertificado = $pdo->query($sqlCertificado);
$datosCertificado = $resultCertificado->fetch(PDO::FETCH_ASSOC);

$nombreCertificado = $datosCertificado['CER_NOMBRE'];
$descripcionCertificado = $datosCertificado['CER_DESCRIPCION'];
$lugarCertificado = $datosCertificado['CER_LUGAR'];
$fechaCertificado = $datosCertificado['CER_FECHA'];
$horasCertificado = $datosCertificado['CER_HORAS'];
...
<body>
    ...
    <div class="emergente__formulario">
        <form action="/Candidato/Model/actualizar.php?id=<?php echo $idUsuario ?>&idCertificacion=<?php echo $idCertificacion ?>" class="emergente__formulario__contenido" method="POST">
            <div class="emergente__formulario__header sombra">
                <h3>Curso o certificado</h3>
            </div>
            <div class="emergente__formulario__campos">
                <div class="campo">
                    <label for="nombreCertificado">Nombre de la certificación y/o curso:</label>
                    <input type="text" name="nombreCertificado" id="nombreCertificado" placeholder="Nombre" value="<?php echo $nombreCertificado; ?>">
                </div>
                <div class="campo">
                    <label for="descripcionCertificado">Descripción de la certificación y/o curso:</label>
                    <textarea name="descripcionCertificado" id="descripcionCertificado" cols="30" rows="5"><?php echo $descripcionCertificado; ?></textarea>
                </div>
                <div class="campo">
                    <label for="lugar">Lugar:</label>
                    <input type="text" name="lugar" id="lugar" placeholder="Udemy, Platzi... etc." value="<?php echo $lugarCertificado; ?>">
                </div>
                <div class="campo">
                    <label for="fechaTermino">Fecha de término:</label>
                    <input type="date" name="fechaTermino" id="fechaTermino" value="<?php echo $fechaCertificado; ?>">
                </div>
                <div class="campo">
                    <label for="horas">Duración (horas):</label>
                    <input type="number" name="horas" id="horas" value="<?php echo $horasCertificado; ?>">
                </div>
            </div>
            <a href="/Candidato/Model/eliminar.php?id=<?php echo $idUsuario ?>&idCertificacion=<?php echo $idCertificacion ?>" class="boton__rojo">Eliminar</a>
            <div class="emergente__formulario__btns">
                <input type="submit" class="boton__verde" value="Actualizar">
